-- Simplify get_customer_profile_details function to only use existing columns
        ...
        'allInteractions', (
          ...
          FROM (
            ...
            SELECT
              ...
              '{}'::text[] AS topics,
              NULL AS next_steps,
              NULL AS outstanding_issues
            FROM public.emails e
            WHERE e.customer_id = c.id
            
            UNION ALL
            
            -- Get all meetings for the customer - using only existing columns
            SELECT
              m.id::text AS interaction_id,
              'Call' AS interaction_type,
              m.meeting_date AS interaction_date,
              m.summary,
              COALESCE(m.sentiment, 'Neutral') AS sentiment,
              -- Convert jsonb arrays to simple text arrays
