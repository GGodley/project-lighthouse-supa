-- Emergency Stop: Stop all thread sync processing (FIXED VERSION)
-- This will mark all active jobs as failed to immediately stop processing
-- Handles different status values and NULL fields safely

-- 1. Mark all running/syncing sync jobs as failed
-- Note: sync_jobs uses 'syncing' not 'running'
UPDATE sync_jobs
SET status = 'failed', 
    details = CASE 
        WHEN details IS NULL THEN 'EMERGENCY STOP: Manually stopped to reduce CPU load'
        ELSE details || ' | EMERGENCY STOP: Manually stopped to reduce CPU load'
    END
...
SET current_stage = 'failed',
    import_error = CASE 
        WHEN import_error IS NULL THEN 'EMERGENCY STOP: Manually stopped to reduce CPU load'
        ELSE import_error || ' | EMERGENCY STOP: Manually stopped to reduce CPU load'
    END,
    next_retry_at = NULL
