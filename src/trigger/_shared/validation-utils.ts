...
import type { GoogleCalendarEvent } from './meeting-utils';
 ...
 */
export function validateMeetingEvent(
  ...
  if (event.attendees) {
    for (let i = 0; i < event.attendees.length; i++) {
      ...
      if (!attendee.email) {
        ...
        errors.push(
          `Attendee at index ${i} has invalid email format: ${attendee.email}`
        );
      }
    }
  }
...
export function hasValidTimezone(event: GoogleCalendarEvent): boolean {
  ...
  if (event.start?.dateTime) {
    ...
    const hasTimezone =
      event.start.dateTime.includes('Z') ||
