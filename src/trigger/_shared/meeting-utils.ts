...
// Ported from Edge Functions for Trigger.dev tasks (Node.js compatible)
...
export type MeetingUrlResult = {
  url: string | null;
  type: 'google_meet' | 'zoom' | 'microsoft_teams' | null;
};
 ...
 * @returns 'google_meet', 'zoom', 'microsoft_teams', or null if unknown
 */
export function detectMeetingType(
  url: string | null | undefined
): 'google_meet' | 'zoom' | 'microsoft_teams' | null {
  if (!url) return null;
  ...
  if (
    lowerUrl.includes('teams.microsoft.com/l/meetup-join/') ||
    lowerUrl.includes('teams.microsoft.com/meetup-join/') ||
    lowerUrl.includes('teams.live.com/meet/')
  ) {
...
export function extractZoomLink(
  ...
  const zoomUrlPattern =
    /https?:\/\/(?:[a-z0-9-]+\.)?zoom\.us\/(?:j|join)\/(?:[a-zA-Z0-9?=_-]+)/gi;
 ...
 */
export function extractGoogleMeetLink(
  location: string | null | undefined,
  description: string | null | undefined
): string | null {
...
/**
 * Extracts Microsoft Teams link from location or description fields
 * @param location The location field from Google Calendar event
 * @param description The description field from Google Calendar event
 * @returns First Teams URL found, or null
 */
export function extractTeamsLink(
  location: string | null | undefined,
  description: string | null | undefined
): string | null {
  // Microsoft Teams URL patterns
  const teamsUrlPattern =
    /https?:\/\/(?:teams\.microsoft\.com\/(?:l\/)?meetup-join\/|teams\.live\.com\/meet\/)[a-zA-Z0-9?=_-]+/gi;

  // Check location field first
  if (location) {
    const locationMatch = location.match(teamsUrlPattern);
    if (locationMatch && locationMatch.length > 0) {
      return locationMatch[0];
    }
  }
  ...
  // Check description field
  if (description) {
    const descriptionMatch = description.match(teamsUrlPattern);
    if (descriptionMatch && descriptionMatch.length > 0) {
      return descriptionMatch[0];
    }
  }
 ...
 * 5. Check location for Teams link
 * 6. Check description for Teams link
 * 7. Check location for Zoom link
 * 8. Check description for Zoom link
...
export function getMeetingUrl(event: GoogleCalendarEvent): MeetingUrlResult {
  ...
  // Some Google Meet/Teams links are only in conferenceData
  if (event.conferenceData?.entryPoints) {
    ...
    if (videoEntry?.uri) {
      ...
      if (type === 'google_meet' || type === 'microsoft_teams') {
        return {
          url: videoEntry.uri,
          type: type,
        };
  ...
  // Priority 5: Check location for Teams link
  const teamsFromLocation = extractTeamsLink(event.location, null);
  if (teamsFromLocation) {
    return {
      url: teamsFromLocation,
      type: 'microsoft_teams',
    };
  }

  // Priority 6: Check description for Teams link
  const teamsFromDescription = extractTeamsLink(null, event.description);
  if (teamsFromDescription) {
    return {
      url: teamsFromDescription,
      type: 'microsoft_teams',
    };
  }

  // Priority 7: Check location for Zoom link
  ...
  // Priority 8: Check description for Zoom link
